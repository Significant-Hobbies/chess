...
const productionPaths = ['src', 'vite.config.ts']
...
function failRegressions(label, observed, baseline) {
  ...
  if (Object.entries(baseline).some(([key, maximum]) => observed[key] < maximum)) {
    console.log(`${label} is below its ceiling; preserve the stricter observed result.`)
  }
}

function changedSourceFiles() {
  const candidates = new Set()
  const workingChanges = run('git', [
    'status',
    '--porcelain',
    '--untracked-files=all',
  ]).stdout.trim()
  const parent = run('git', ['rev-parse', '--verify', 'HEAD^'], { allowFailure: true })
  if (!workingChanges && parent.status === 0) {
...
function checkDuplication() {
  const outputDirectory = mkdtempSync(join(tmpdir(), 'chess-jscpd-'))
  run('pnpm', [
    'exec',
    'jscpd',
    ...productionPaths,
    ...
    'strict',
    '--ignore',
    '**/*.test.*,**/*.d.ts,**/node_modules/**,**/dist/**,scripts/check-code-health.mjs',
    '--reporters',
    'json',
    '--output',
    outputDirectory,
...
function checkDependencies() {
  ...
  const critical = severe.filter((advisory) => advisory.severity === 'critical')
  const high = severe.filter((advisory) => advisory.severity === 'high')
  console.log(`Dependencies: ${critical.length} critical, ${high.length} high advisories.`)
  if (severe.length > 0) {
    throw new Error(
...
function checkSuppressions() {
  const files = ['src', 'tests', 'scripts'].flatMap((root) =>
    sourceFiles(resolve(projectRoot, root))
  )
  files.push(resolve(projectRoot, 'vite.config.ts'), resolve(projectRoot, 'playwright.config.ts'))
  const matches = files
...
function checkHygiene() {
  ...
  const generated = run('git', ['ls-files', '--others', '--exclude-standard'])
    ...
    .filter(Boolean)
    .filter((file) =>
      /(^|\/)(?:coverage|dist|build|\.next|\.open-next|\.wrangler|test-results|playwright-report)(?:\/|$)|(?:^|\/)\.DS_Store$|\.tsbuildinfo$/u.test(
        file
      )
    )
